...
class UserRepository {
  ...
  Future<int> login(
    ...
    body["password"] = password;
    body["provider"] = provider.toString();
    body["fcm_token"] = await _sp.getString("firebase_token");
    log("Firebase token: " + body["fcm_token"]);
    ...
    if (response != null) {
      ...
      } else if (response["message"] == "user is already connected") {
        return 0;
      }
    }
    return -1;
  ...
  Future<int> logout() async {
    ...
    if (bearer != "") {
      ...
      Map<String, dynamic> response = await _userApi.logout(body);
  ...
  Future sendPoints(String points) async{
    SharedPreferences _sp = await SharedPreferences.getInstance();
    String bearer = _sp.getString("bearer") ?? "";
    ...
    Map<String, dynamic> response = await _userApi.postCoupons(headers, body);
    if(response != null) {
