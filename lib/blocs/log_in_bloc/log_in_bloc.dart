...
class LogInBloc extends Bloc<LogInEvent, LogInState> {
  ...
  LogInBloc(this._userRepository) : super(LogInState.initial()) {
    on<LogInEvent>((event, emit) async {
      await event.when(forgotPassword: () {
        ...
        emit(
          state.copyWith(loginStatus: LoginStatus.register, errorMessage: ""),
        );
      }, facebookLoginEvent: (String email, String password) async {
        UserCredential userCredential =
            await FacebookSignInHelper().signInWithFacebook();
        if (await _userRepository.login(
                email: userCredential.user!.email, password: "", provider: 2) !=
            1) {
          if (await _userRepository.register(
      ...
      }, register: (String email, String password, String firstName,
        ...
        if (response == 1) {
          ...
          emit(
            state.copyWith(
                loginStatus: LoginStatus.registerFailed,
                errorMessage:
                    "Υπήρχε πρόβλημα κατά την εγγραφή! Προσπαθήστε ξανά!"),
          );
        }
        ...
        int response = await _userRepository.login(
            email: email, password: password, provider: 1);
        if (response !=
            1) {
          if(response == 0) {
            emit(
              state.copyWith(
                  loginStatus: LoginStatus.logInExist,
        ...
        if (await _userRepository.login(
          ...
          if (await _userRepository.register(
              ...
              1) {
            await _userRepository.login(
                email: userCredential.user!.email, password: "", provider: 3);
          }
          emit(
            state.copyWith(
                loginStatus: LoginStatus.logInSuccessful,
        ...
        if (state.loginStatus == LoginStatus.register) {
          emit(
            state.copyWith(loginStatus: LoginStatus.login),
          );
        }
      });
    });
