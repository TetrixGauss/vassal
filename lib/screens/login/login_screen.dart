    ...
    return BlocConsumer<LogInBloc, LogInState>(
        listener: (context, state) {
          if (state.loginStatus == LoginStatus.logInFailed) {
            ...
            AppToast.showMessage(state.errorMessage );
          } else if (state.loginStatus == LoginStatus.registerSuccessful) {
            AppToast.showMessage("H εγγραφή σας πραγματοποιήθηκε!");
          } else if (state.loginStatus == LoginStatus.logInExist) {
              AppToast.showMessage("Ο χρήστης είναι ήδη συνδεδεμένος με κάποια άλλη συσκευή!");
          }
        },
