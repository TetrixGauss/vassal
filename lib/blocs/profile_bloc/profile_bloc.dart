...
class ProfileBloc extends Bloc<ProfileEvent, ProfileState> {
  ...
  ProfileBloc(
      this._userRepository, this._appointmentRepository, this._databaseHelper)
    ...
    on<ProfileEvent>((event, emit) async {
      await event.when(initProfile: () async {
        ...
        emit(state.copyWith(
            ...
            user: await _getUser(),
            appointments: await _getAppointments()));
      }, logOut: () async {
        if (await _userRepository.logout() == 1) {
          _databaseHelper.deleteEverything();
          emit(state.copyWith(profileStatus: ProfileStatus.logOut));
        } else {
          emit(state.copyWith(profileStatus: ProfileStatus.logOutFailed));
        }
      }, addAppointment: () {
